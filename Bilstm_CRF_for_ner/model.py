...
class BiLSTM_CRF(object):
    ...
    def run_one_epoch(self, sess, train_no_shuffle,train, dev, tag2label, epoch, saver,model_path):
        ...
        if model_path:
            ...
            if epoch==0:
                ...
                f1score_valid = self.evaluate(label_list_dev, seq_len_list_dev, dev,self.test_path, epoch)
                label_list_train, seq_len_list_train = self.dev_one_epoch(sess, train)
                f1score_train = self.evaluate(label_list_train, seq_len_list_train, train, self.train_path, epoch)
        ...
        for step, (seqs, labels) in enumerate(batches):
            ...
            _, loss_train, summary, step_num_ = sess.run([self.train_op, self.loss, self.merged, self.global_step],
                                                         feed_dict=feed_dict)
            tjl=self.batch_size
            if self.batch_size<=10:tjl=self.batch_size*10
            if step + 1 == 1 or (step + 1) % tjl == 0 or step + 1 == num_batches:
                print('logger info')
                print(
        ...
        f1score_train=self.evaluate(label_list_train, seq_len_list_train, train_no_shuffle,self.train_path, epoch)
    ...
    def evaluate(self, label_list, seq_len_list, data,raw_data=None,epoch=None):
        """
        ...
        dg_file_path=os.path.join(self.result_path, 'dg_file')
        result_file_path = os.path.join(self.result_path, 'result_file' + epoch_num)
        metric_path = os.path.join(self.result_path, 'result_metric_' + epoch_num)
        f1score = conlleval(model_predict, dg_file_path, metric_path,raw_data,result_file_path)
        if len(data)>10000:
            self.logger.info('epochs: {} 训练集f1score: {}'.format(epoch + 1, f1score))
        else:
            self.logger.info('epochs: {} 验证集f1score: {}'.format(epoch+1,f1score))
        return f1score
