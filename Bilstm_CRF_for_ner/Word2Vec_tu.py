...
with open('data_path//DaGuang//dr_d_td_zh.pkl','rb') as f1:
    dictionary_reverse=pickle.load(f1)
    dictionary=pickle.load(f1)
    data_num=pickle.load(f1)

print('word总数量：',len(data_num))     #111997581  #111997581      #03版  112972260
print('字典的长度：',len(dictionary))
assert len(dictionary)==len(dictionary_reverse)

assert dictionary['[<UNK>]']==len(dictionary)-1
...
vocabulary_size=len(dictionary)           #防止总共不同的词数少于设置总词长，导致后面的embedding个数超过字典的总长，导致报keyerror
batch_size=120
embedding_size=500
neg_samples=75  #一般num_skip=2*skip_window  batch_size需要能整除num_skip
num_skip=10     ## How many times to reuse an input to generate a label.
...
class Word2vec(object):
    ...
    def training(self,sess,save_path=None,load_path=None):
        ...
        for epoch in range(10):
            steps=len(data_num)//batch_size
            if steps<len(data_num)/batch_size:
                steps+=1
                print('steps:',steps)
            ...
            #每隔500次打印一次
                if step%1000==0:
                    print("=====================================================")
                    print('learning_rate:{} loss:{} epoch:{}'.format(learn_rate, loss, epoch))
            ...
            final_embedding = self.normalized_embeddings.eval()
            if not os.path.exists('data_path//DaGuang'): os.mkdir('data_path//DaGuang')
            with open('data_path//DaGuang//zh_embeddings{}500.pkl'.format(steps), 'wb') as f:
    ...
    def inference(self,sess,load_path,is_save_vector=False):
        ...
        if is_save_vector:
            ...
            with open(r'data_path\DaGuang\embeddings_300_106.pkl','wb') as f:
                pickle.dump(embed,f)
            print('成功保存词向量！')
        while 1:
            word = input('请输入：')
...
with tf.Session() as sess:
    ...
    model.build_graph()
    if not os.path.exists('model_path//DaGuang//word2vec_tu_model'):os.makedirs('model_path//DaGuang//word2vec_tu_model')
    model.training(sess=sess,save_path='model_path//DaGuang//word2vec_tu_model\zh_save_net.ckpt',load_path=None)
#加载模型继续训练
...
#     model=Word2vec(mode='train')
#     model.build_graph()
#     model.training(sess=sess,save_path=r'300dim_03\word2vec_pre_model\save_net.ckpt'
#                    )
#预测
# tf.reset_default_graph()
# with tf.Session() as sess:
...
#     model.build_graph()
#     model.inference(sess=sess,load_path=r'model_path\DaGuang\word2vec_tu_model\save_net.ckpt-1000001',is_save_vector=True)
